...
try {
  ...
  console.error('Shimi Looks init failed:', error);
  document.addEventListener('DOMContentLoaded', () => {
    const title = document.getElementById('lookTitle');
    const vibe = document.getElementById('lookVibe');
    if(title) title.textContent = 'יש תקלה קטנה בטעינת הלוק';
    if(vibe) vibe.textContent = 'נסי לרענן. אם זה ממשיך, העלי את גרסת v4 מחדש.';
  });
}
...
function init(){
  ...
  // Disable old service workers so GitHub Pages will not serve a stale cached version.
